...
//select am.user_id, count(view) as view_count from ad_metrics as am where view > 0 group by user_id;
//select am.user_id, count(click) as click_count from ad_metrics as am where click > 0 group by user_id;

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class AdMetrics extends CI_Controller {
    ...
    function AdMetrics() {
        ...
        $this->load->model('ad_page_model', 'ad_pages', true);
    ...
    public function index($ad_metrics_id = null) {
        ...
        $header['headscript'] = $this->functions->jsScript('ad_metrics.js');
        
        if(!is_null($ad_metrics_id)){        	
        	try {
        		$pagination_config['base_url'] = '/administrator/ad_metrics/index//'.$page;
        		$pagination_config['total_rows'] = $this->ad_metrics->countAll();
        		$pagination_config['per_page'] = 5;
        		$pagination_config['cur_page'] = $page;
        		$pagination_config['use_page_numbers'] = TRUE;
        		$this->pagination->initialize($pagination_config);
        		//$ad_metrics = $this->ad_metrics->fetchAll(array('orderby' => 'ad_metrics_id DESC', 'limit' => $pagination_config['per_page'], 'offset' => $page)); 
        		
        		//foreach(ad_metrics as $ad_metric){ 
        		//	$ad_metric->product = $this->product->fetchAll(array('where' => 'product_id = '.$listing->product_id))[0];        			
        		//}
        	...
        	$ad_metrics = $this->ad_metrics->fetchAll(array('where' => 'ad_metrics_id = '.$ad_metrics_id));        	
        }
        
        $body['ad_metrics'] = $ad_metrics;
        $body['admin_menu'] = $this->load->view('admin/admin_menu', null, true);
        $this->load->view('administrator/template/header', $header);
        $this->load->view('administrator/ad_metrics', $body);
